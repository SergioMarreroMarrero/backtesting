...
def run(exchange: str, symbol: str, strategy: str, tf: str, from_time: int, to_time: int):
    ...
    if strategy == "obv":
        ...
        data = h5_db.get_data(symbol, from_time, to_time)
        data = resample_timeframe(data, tf)
        pnl, max_drawdown = ichimoku.backtest(data, tenkan_period=params.tenkan_period, kijun_period=params.kijun_period)
        ...
        data = h5_db.get_data(symbol, from_time, to_time)
        data = resample_timeframe(data, tf)
        pnl, max_drawdown = support_resistance.backtest(data,
